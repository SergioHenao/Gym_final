...
<html lang="es">
  ...
  <body>
      ...
      <div class="container-fluid position-relative">
        <nav class="navbar navbar-expand-lg">
          <div class="site-logo">
            ...
            <!-- Codigo para la autenticación de usuarios y sus roles -->
            @if(auth::user())
            <!-- Condicional para el rol de admin -->
            @if(auth()->user()->rol=="admin")
            @include('modulos.Admin')
            @endif
            <!-- Condicional para el rol de entrenador -->
            @if(auth()->user()->rol=="entrenador")
            @include('modulos.Coach')
            @endif
            @endif

            
          </div>
          <!-- Boton de responsive para movil o tablet -->
          <button
    ...
    <br>
    <br>
    <!-- Autenticador para el usuario -->
    @if(auth::user())
    <!-- Condicional para los usuarios con el rol user -->
    @if(auth()->user()->rol=="user")
    @include('modulos.Tarjetas')
    @endif
    @yield('content')
    @endif
    ...
    <!-- Footer -->
    <footer class="cta-section text-center py-3 theme-bg-dark position-relative">
